...
public class Fraction {
	...
	public String toString(){
		...
		if (separatorPosition == result.length()) return result + ".0";
		int newlength = 0;
		if(result.length() == 2) newlength = result.length();
		else newlength = result.length() - 1;
		return result.substring(0, separatorPosition) + Comma + result.substring(separatorPosition, newlength);  
	}
	...
	public Fraction divide(long allCp) {
		Fraction f = new Fraction(BigInteger.valueOf(allCp), BigInteger.ONE);
	...
	public boolean smaller(Fraction f) {
		return this.enumerator.multiply(f.denominator).compareTo(this.denominator.multiply(f.enumerator)) < 0;
	}
}