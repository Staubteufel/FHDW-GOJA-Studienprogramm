...
public class StudentServerLst_StudentServersProxi extends PersistentMapProxi<String,StudentServer4Public>{

	private StudentServerLst owner;

	private Hashtable<String, StudentServer4Public> data;
	private boolean inSync;

	public StudentServerLst_StudentServersProxi(StudentServerLst owner) {
		this.owner = owner;
	...
	public synchronized StudentServer4Public put(String key, StudentServer4Public entry) throws PersistenceException {
		...
		if (!this.owner.isDelayed$Persistence()){
			entry.store();
			result = ConnectionHandler.getTheConnectionHandler().theStudentServerLstFacade.studentServersAdd(owner.getId(), key, entry);
			if (this.inSync){
	...
	public synchronized StudentServer4Public get(String key) throws PersistenceException {
		...
		if (result == null && !this.owner.isDelayed$Persistence()) {
			result = ConnectionHandler.getTheConnectionHandler().theStudentServerLstFacade.studentServersGet(owner.getId(), key);
			if (result != null)
	...
	public synchronized StudentServer4Public remove(String key) throws PersistenceException {
		...
		if (!this.owner.isDelayed$Persistence()){
			result = ConnectionHandler.getTheConnectionHandler().theStudentServerLstFacade.studentServersRem(this.owner.getId(), key);
		}
		return result;
	}
	protected void removeEntry(StudentServer4Public entry) throws PersistenceException {
    	...
    	if (!this.owner.isDelayed$Persistence()) {
      		ConnectionHandler.getTheConnectionHandler().theStudentServerLstFacade.studentServersRemEntr(((PersistentListEntryProxi)entry).getListEntryId());
    	}
  	}
	@SuppressWarnings("unchecked")
	public StudentServerLst_StudentServersProxi copy(StudentServerLst owner) throws PersistenceException {
		StudentServerLst_StudentServersProxi result = new StudentServerLst_StudentServersProxi(owner);
	...
	private void synchroniseWithDatabase() throws PersistenceException {
		if(!this.inSync){
			this.data = ConnectionHandler.getTheConnectionHandler().theStudentServerLstFacade.studentServersGetValues(owner.getId());
			this.inSync = true;
		}
	}
	...
	public void store() throws PersistenceException {
		...
		while (keys.hasNext()){
			...
			data.get(key).store();
			ConnectionHandler.getTheConnectionHandler().theStudentServerLstFacade.studentServersAdd(owner.getId(), key, data.get(key));
		}
