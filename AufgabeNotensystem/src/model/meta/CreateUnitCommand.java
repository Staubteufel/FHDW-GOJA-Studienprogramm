...
public class CreateUnitCommand extends PersistentObject implements PersistentCreateUnitCommand{
    ...
    public static CreateUnitCommand4Public getById(long objectId) throws PersistenceException{
        long classId = ConnectionHandler.getTheConnectionHandler().theCreateUnitCommandFacade.getClass(objectId);
        return (CreateUnitCommand4Public)PersistentProxi.createProxi(objectId, classId);
    }
    
    public static CreateUnitCommand4Public createCreateUnitCommand(String name,common.Fraction creditPoints,java.sql.Date createDate,java.sql.Date commitDate) throws PersistenceException{
        return createCreateUnitCommand(name,creditPoints,createDate,commitDate,false);
    }
    
    public static CreateUnitCommand4Public createCreateUnitCommand(String name,common.Fraction creditPoints,java.sql.Date createDate,java.sql.Date commitDate,boolean delayed$Persistence) throws PersistenceException {
        ...
        if(delayed$Persistence){
            result = ConnectionHandler.getTheConnectionHandler().theCreateUnitCommandFacade
                .newDelayedCreateUnitCommand(name,creditPoints);
            result.setDelayed$Persistence(true);
        }else{
            result = ConnectionHandler.getTheConnectionHandler().theCreateUnitCommandFacade
                .newCreateUnitCommand(name,creditPoints,-1);
        }
        ((PersistentCreateUnitCommand)result).setMyCommonDate((PersistentCommonDate)CommonDate.createCommonDate(createDate, createDate));
        return result;
    }
    ...
    public CreateUnitCommand(String name,common.Fraction creditPoints,Invoker invoker,PersistentAdminServer commandReceiver,PersistentCommonDate myCommonDate,long id) throws PersistenceException {
    ...
    public void store() throws PersistenceException {
        ...
        if (this.getClassId() == 161) ConnectionHandler.getTheConnectionHandler().theCreateUnitCommandFacade
            .newCreateUnitCommand(name,creditPoints,this.getId());
        super.store();
    ...
    public void setCreditPoints(common.Fraction newValue) throws PersistenceException {
        if(!this.isDelayed$Persistence()) ConnectionHandler.getTheConnectionHandler().theCreateUnitCommandFacade.creditPointsSet(this.getId(), newValue);
    ...
    public void execute() 
        ...
        try{
			this.commandReceiver.createUnit(this.getName(), this.getCreditPoints());
		}
