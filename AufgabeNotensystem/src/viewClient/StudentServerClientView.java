...
import viewClient.AdminServerClientView.MenuItemVisitor;
...
public class StudentServerClientView extends BorderPane implements ExceptionAndEventHandler{

	private ConnectionMaster 		 connection;
	private ExceptionAndEventHandler parent;	
	private StudentServerView 		 		 service;

	/**
	 * This is the default constructor
	 */
	public StudentServerClientView( ExceptionAndEventHandler parent, StudentServerView service) {
		super();
		this.parent = parent;
	...
	private StudentServerView getService(){
		return this.service;
	}
	...
	protected void setDetailsTable( Anything object) {
		...
		if (object == null && this.getConnection() != null) object = this.getConnection().getStudentServerView();
		if (object == null){
	...
	public StudentServerConnection getConnection(){
        	return (StudentServerConnection)this.connection;
	}
	...
	public void handleRefresh(){
		Platform.runLater(new Runnable() {
			public void run(){
				try {
					getNavigationTree().refreshTree();
					java.util.Vector<?> errors = getConnection().getStudentServerView().getErrors();
	...
	**/
	public void initializeConnection(){
		Platform.runLater( new  Runnable() {
			public void run() {
				getNavigationTree().setModel((TreeModel) getConnection().getStudentServerView());	
    ...
    interface MenuItemVisitor{
        ImageView handle(FetchStudentMPRMTRApprPRMTRMenuItem menuItem);
    }
    private abstract class StudentServerMenuItem extends MenuItem{
        private StudentServerMenuItem(){
            this.setGraphic(getIconForMenuItem(this));
        }
        abstract protected ImageView accept(MenuItemVisitor visitor);
    }
    private class FetchStudentMPRMTRApprPRMTRMenuItem extends StudentServerMenuItem{
    ...
    private ContextMenu getContextMenu(final ViewRoot selected, final boolean withStaticOperations, final Point2D menuPos) {
        ...
        if (selected != null){
            try {
                this.setPreCalculatedFilters(this.getConnection().studentServer_Menu_Filter((Anything)selected));
            } catch (ModelException me){
                this.handleException(me);
                return result;
            }
            if (selected instanceof ApprView){
                item = new FetchStudentMPRMTRApprPRMTRMenuItem();
                ...
                item.setOnAction(new EventHandler<ActionEvent>(){
                    public void handle(javafx.event.ActionEvent e) {
                        ...
                        if (buttonResult.get() == ButtonType.OK) {
                            try {
                                ViewRoot result = (ViewRoot)getConnection().fetchStudentM((ApprView)selected);
                                getConnection().setEagerRefresh();
                                ReturnValueView view = new ReturnValueView(result, new javafx.geometry.Dimension2D(getNavigationPanel().getWidth()*8/9,getNavigationPanel().getHeight()*8/9), StudentServerClientView.this);
	...
	private ImageView getIconForMenuItem(StudentServerMenuItem menuItem){
//		return new ImageView(new javafx.scene.image.Image("/viewResources/default.gif"));
		return menuItem.accept(new MenuItemVisitor() {

			@Override
			public ImageView handle(FetchStudentMPRMTRApprPRMTRMenuItem menuItem) {
