...
//		});
		...
		this.setCenter(this.getMainTabPane());
		try {
			java.util.Iterator<ServiceView> serviceIterator = this.getConnection().getServerView().getServices().iterator();
			while (serviceIterator.hasNext()) {
				ServiceView current = serviceIterator.next();
//		ServiceView current = this.getConnection().getServerView().getService();
				current.accept(new ServiceVisitor() {

					@Override
					public void handleAdminServer(AdminServerView adminServer) throws ModelException {
						AdminServerClientView view = new AdminServerClientView(ServerClientView.this, adminServer);
						adminServer.connectAdminServer(ServerClientView.this.getConnection(), view);
						Tab newTab = new Tab(adminServer.toString(), view);
						getMainTabPane().getTabs().add(newTab);
					}

					@Override
					public void handleStudentServer(StudentServerView studentServer) throws ModelException {
						StudentServerClientView view = new StudentServerClientView(ServerClientView.this, studentServer);
						studentServer.connectStudentServer(ServerClientView.this.getConnection(), view);
						Tab newTab = new Tab(studentServer.toString(), view);
						getMainTabPane().getTabs().add(newTab);
					}
				});
			}
			this.getConnection().refresherStop();
		} catch (ModelException e) {
			this.handleException(e);
		}
	}
